...
public struct SystemScopeFilterBar: View {
    ...
    private func scopeChip(
        ...
        Button(action: {
            ...
            .padding(.vertical, 3)
            .background(
                RoundedRectangle(cornerRadius: Metric.chipRadius, style: .continuous)
                    .fill(isActive ? (color == .primary ? Color.primary.opacity(0.12) : color.opacity(0.14)) : Color.primary.opacity(0.04))
            )
            .overlay(
                RoundedRectangle(cornerRadius: Metric.chipRadius, style: .continuous)
                    .strokeBorder(
                        isActive ? (color == .primary ? Color.primary.opacity(0.2) : color.opacity(0.5)) : Color.primary.opacity(0.08),
                        lineWidth: 1
                    )
            )
        }
